        ...
        if (!empty($GLOBALS['table']) && $cfg['ShowTooltip']) {
            ...
            if ($result) {
                ...
                $tooltip    = (empty($tbl_status['Comment']))
                            ? ''
                            : htmlspecialchars($tbl_status['Comment']) . ' ';
    ...
    function PMA_checkParameters($params, $die = TRUE) {
        ...
        foreach ($params AS $param) {
            if (!isset($GLOBALS[$param])) {
                $error_message .= htmlspecialchars($reported_script_name) . ': Missing parameter: ' . $param . ' <a href="./Documentation.html#faqmissingparameters" target="documentation"> (FAQ 2.8)</a><br />';
                $found_error = TRUE;
            }
        }
        if ($found_error) {
